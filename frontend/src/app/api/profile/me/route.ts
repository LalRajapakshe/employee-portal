...
export async function GET(request: NextRequest) {
  ...
  const result = await callCoreApi<ProfilePayload>('/api/employees/me', {
    method: 'GET',
  ...
  const response = NextResponse.json({ ...result.payload, correlationId: result.correlationId ?? correlationId }, { status: result.status });
  response.headers.set(CORRELATION_ID_HEADER, result.correlationId ?? correlationId);
  return response;
}
