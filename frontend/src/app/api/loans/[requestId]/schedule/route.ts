...
import type { LoanRepaymentScheduleItem } from '@/types/loan';

type Payload = { success: boolean; data?: LoanRepaymentScheduleItem[]; message?: string; correlationId?: string };
...
export async function GET(request: NextRequest, { params }: { params: Promise<{ requestId: string }> }) {
  ...
  const result = await callCoreApi<Payload>(`/api/loans/${requestId}/schedule`, { headers: buildCoreSessionHeaders(session, correlationId) });
