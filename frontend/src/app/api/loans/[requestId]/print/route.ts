...
type Payload = { success: boolean; data?: LoanPrint; message?: string; correlationId?: string };
...
export async function GET(request: NextRequest, { params }: { params: Promise<{ requestId: string }> }) {
  ...
  const result = await callCoreApi<Payload>(`/api/loans/${requestId}/print`, { headers: buildCoreSessionHeaders(session, correlationId) });
