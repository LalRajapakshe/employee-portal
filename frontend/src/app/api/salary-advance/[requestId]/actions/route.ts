...
type Payload = { success: boolean; data?: SalaryAdvanceRequest; message?: string; correlationId?: string };
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ requestId: string }> }) {
  ...
  const result = await callCoreApi<Payload>(`/api/salary-advance/${requestId}/actions`, {
    method: 'POST',
