...
type Payload = { success: boolean; data?: ApprovalInboxItem[]; message?: string; correlationId?: string };
...
export async function GET(request: NextRequest) {
  ...
  const result = await callCoreApi<Payload>('/api/approvals/inbox', { headers: buildCoreSessionHeaders(session, correlationId) });
